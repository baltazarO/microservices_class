...
def test_post_vote_count_goes_down_after_voting(client, test_user, single_post):
    single_post.vote_count = 4
    assert single_post.vote_count == 4
    single_post.down_vote(test_user)
    assert single_post.vote_count == 3


def test_post_vote_after_voting_down(client, test_user, single_post):
    single_post.vote_count = 4
    single_post.down_vote(test_user)
    single_post.down_vote(test_user)
    assert single_post.vote_count == 3
...
def test_repr_function(client, test_user, single_post):
    c1 = single_post.add_comment("Important insight!", test_user)
    assert c1.__repr__() == "<Comment id 1 - Important insight!>"
...
def test_comments_adjust_vote(client, test_user, single_post):
    c1 = single_post.add_comment("Important insight!", test_user)
    c1.vote_count = None
    assert c1.vote_count == None
    c1.adjust_vote(1)
    assert c1.vote_count == 1


def test_comments_can_be_voted_down(client, test_user, single_post_with_comment):
    comment = single_post_with_comment.comments[0]
    new_user = User(username="robot", email="robot@gmail.com")
    db.session.add(new_user)
    db.session.commit()
